...
## Define genomic localization of BCOR Peaks (Figure Sup2D)

```{r make_tss_prom,eval=F}
txdb<-makeTxDbFromGFF("../hESC/gencode.v27.basic.annotation.gff3",format = "gff3")
ens<-exonsBy(txdb,by="gene")
names(ens)<-substr(names(ens),1,15)
...
Annotation <- getAnnotation(ensembl, featureType="transcript")
hgnc<-getBM(filters="ensembl_gene_id",values=names(ens),
     attributes=c("ensembl_gene_id","wikigene_description","ensembl_transcript_id","hgnc_symbol","transcript_biotype"),mart=ensembl)
...
#ENCODE BLACKLIST
bl_hg38<-import("../hESC/blacklist_hg38_ENCFF419RSJ.bed")
seqlevelsStyle(bl_hg38)<-"NCBI"

length(wt_peaks<-broadPeakToGRanges("../hESC/BCOR_H1_wt_081817_broad_peaks.broadPeak","wt"))
length(k2bko_peaks<-broadPeakToGRanges("../hESC/BCOR_H1_k2bko_081817_broad_peaks.broadPeak","k2bko"))
length(rnf2plus_peaks<-broadPeakToGRanges("../hESC/BCOR_H1_rnf2rplus_081817_broad_peaks.broadPeak","rnf2plus"))
length(rnf2minus_peaks<-broadPeakToGRanges("../hESC/BCOR_H1_rnf2rminus_081817_broad_peaks.broadPeak","rnf2minus"))

annotate_peaks<-function(gr) {
  seqlevelsStyle(gr)<-"NCBI"
  gr<-gr[!gr %over% bl_hg38]
  gr_anno<-annotatePeakInBatch(gr, AnnotationData=Annotation, maxgap=0L,select="first",output="shortestDistance")
  seqlevelsStyle(gr_anno)<-"NCBI"
  stopifnot(wls(gr)==wls(gr_anno))

  idx<-match(gr_anno$feature,hgnc$ensembl_transcript_id)
  gr$symbol<-hgnc[idx,"hgnc_symbol"]
  gr$ensembl_gene_id<-hgnc[idx,"ensembl_gene_id"]
  gr_anno$class<-ifelse(gr_anno$insideFeature %in% c("includeFeature","inside","overlapEnd","overlapStart"),"proximal",
                  ifelse(gr_anno$insideFeature %in% c("upstream","downstream") & abs(gr_anno$shortestDistance) < 10000,"distal","intergenic"))
  idx2<-match(names(gr),gr_anno$peak)
  gr$class<-gr_anno[idx2]$class
  gr$hgnc<-gr_anno[idx2]$hgnc
  gr$ensembl<-gr_anno[idx2]$ensembl_gene_id
  return(gr)
}

#Annotate
wt_peaks<-annotate_peaks(wt_peaks)
k2bko_peaks<-annotate_peaks(k2bko_peaks)
rnf2plus_peaks<-annotate_peaks(rnf2plus_peaks)
rnf2minus_peaks<-annotate_peaks(rnf2minus_peaks)

#Summarize Classes
(df<-data.frame(wt=as.numeric(table(wt_peaks$class)),k2bko=as.numeric(table(k2bko_peaks$class)),
           rnf2plus=as.numeric(table(rnf2plus_peaks$class)),rnf2minus=as.numeric(table(rnf2minus_peaks$class)),
          row.names=names(table(wt_peaks$class))))

df %>%
  tibble::rownames_to_column(var = "class") %>%
  tidyr::gather(celltype,peaks,wt:rnf2minus) %>%
  mutate(celltype=factor(celltype,levels=c("wt","rnf2plus","rnf2minus","k2bko"))) %>%
  ggplot(aes(x=celltype,y=peaks,fill=class)) + geom_bar(stat="identity") +
  ggtitle("BCOR Peak Distribution in WT and LOF Datasets") +
  theme_bw() +xlab("") +ylab("Number of Peaks")

```


## Define TSS Regions & Measure counts for each ChIP Experiment
...
fls<-c(fls,"H1_egs_input_P36_Index6_S10.fastq.bam","H1_k2bko_input_P36_Index8_S12.fastq.bam","Input_DNA_zheng_CAGATC_bothruns.R1_trimmed.fastq.bam")
#fls<-c("../hESC/Input_DNA_zheng_CAGATC_bothruns.R1_trimmed.fastq.bam","../hESC/H3K36me2_ChIP_zheng_GCCAAT_bothruns.R1_trimmed.fastq.bam")
...
load("../hESC/tss_counts.rdata")
load("tss_prom.rdata")

tss_counts<-cbind(assays(tss_counts_ncbi)$counts,assays(tss_counts_ucsc)$counts,assays(tss_counts_k36me2)$counts)
colnames(tss_counts)<-c("BCOR","KDM2B","RNF2","PCGF1","RYBP","CGI","H2AUb","H3K36me2","Ser5p","Ser7p","INPUT_EGS","INPUT","SUZ12","H3K27me3","CBX2","DNAse1","H3K4me3","H3K79me2","INPUT_H3K36me2","H3K36me2_rep")
all.equal(tss_counts[,"H3K36me2"],tss_counts[,"H3K36me2_rep"])
tss_counts<-tss_counts[,c("INPUT_EGS","INPUT","INPUT_H3K36me2","BCOR","KDM2B","RNF2","PCGF1","RYBP","CGI","H2AUb","H3K36me2","SUZ12","H3K27me3","CBX2","DNAse1","H3K4me3","H3K79me2","Ser5p","Ser7p")]

tsswide_counts<-cbind(assays(tsswide_counts_ncbi)$counts,assays(tsswide_counts_ucsc)$counts,assays(tsswide_counts_k36me2)$counts)
colnames(tsswide_counts)<-c("BCOR","KDM2B","RNF2","PCGF1","RYBP","CGI","H2AUb","H3K36me2","Ser5p","Ser7p","INPUT_EGS","INPUT","SUZ12","H3K27me3","CBX2","DNAse1","H3K4me3","H3K79me2","INPUT_H3K36me2","H3K36me2_rep")
all.equal(tsswide_counts[,"H3K36me2"],tsswide_counts[,"H3K36me2_rep"])
tsswide_counts<-tsswide_counts[,c("INPUT_EGS","INPUT","INPUT_H3K36me2","BCOR","KDM2B","RNF2","PCGF1","RYBP","CGI","H2AUb","H3K36me2","SUZ12","H3K27me3","CBX2","DNAse1","H3K4me3","H3K79me2","Ser5p","Ser7p")]
...
## K-Means clustering and heatmap plots for TSS in each group
```{r kmeans,eval=F}
#colnames(mcols(tss_prom))
#  Went back and added Input samples to this table which are excluded from the clustering data
x<-mcols(tss_prom)[,8:23]
kx<-kmeans(x,3)
...
## Correlation Matrix (Figure 2E)

For the paper the correlation values were clustered using TreeView and visualized using Cluster software packages.
```{r correlation_matrices,eval=F}
...
## GO terms for genes defined by clustering (Figure Sup 2E)
...
### Expression Analysis - shRNA and Timecourse Data (Figure 2F)
...
### Make tables of FPKM and Counts
...
##Differential Expression - Timecourse w/o replicates
```{r}
...
length(kdm2b_genes<-tss_classified$ensembl[tss_classified$k2bKO_deg!="NR" ])  #kdm2b degs

#skip this section because we need to assign all genes
...
#dim(f_diff<-f[,c(1:7,16:22,25,14,15)])
#table(apply(f_diff,1,function(x) sum(x > 2.5)))
#dim(f_diff<-f_diff[apply(f_diff,1,function(x) sum(x > 2.5)) > 0,])
f_diff<-as.data.frame(f_diff)
#get fold change over t=0
temp<-log2(f_diff+0.01)
temp<-cbind(temp[,2:7]-temp[,1],temp[,9:14]-temp[,8],temp[,16:17]-temp[,15])

table(f_diff$bcor_deg<-ifelse((temp$bcor_1 > 1 | temp$bcor_2 > 1 | temp$bcor_3 > 1 | temp$bcor_4 > 1 | temp$bcor_5 > 1 | temp$bcor_6 > 1) & 
                                (f_diff$bcor_0 > 2.5 | f_diff$bcor_1 > 2.5 | f_diff$bcor_2 > 2.5 | f_diff$bcor_3 > 2.5 | f_diff$bcor_4 > 2.5 | f_diff$bcor_5 > 2.5 | f_diff$bcor_6 > 2.5),"UP",
                             ifelse((temp$bcor_1 < -1 | temp$bcor_2 < -1 | temp$bcor_3 < -1 | temp$bcor_4 < -1 | temp$bcor_5 < -1 | temp$bcor_6 < -1) &
                                      (f_diff$bcor_0 > 2.5 | f_diff$bcor_1 > 2.5 | f_diff$bcor_2 > 2.5 | f_diff$bcor_3 > 2.5 | f_diff$bcor_4 > 2.5 | f_diff$bcor_5 > 2.5 | f_diff$bcor_6 > 2.5), "DN","NR")))

table(f_diff$rab_deg<-ifelse((temp$r1b_1 > 1 | temp$r1b_2 > 1 | temp$r1b_3 > 1 | temp$r1b_4 > 1 | temp$r1b_5 > 1 | temp$r1b_6 > 1) & 
                                (f_diff$r1b_0 > 2.5 | f_diff$r1b_1 > 2.5 | f_diff$r1b_2 > 2.5 | f_diff$r1b_3 > 2.5 | f_diff$r1b_4 > 2.5 | f_diff$r1b_5 > 2.5 | f_diff$r1b_6 > 2.5),"UP",
                             ifelse((temp$r1b_1 < -1 | temp$r1b_2 < -1 | temp$r1b_3 < -1 | temp$r1b_4 < -1 | temp$r1b_5 < -1 | temp$r1b_6 < -1) &
                                      (f_diff$r1b_0 > 2.5 | f_diff$r1b_1 > 2.5 | f_diff$r1b_2 > 2.5 | f_diff$r1b_3 > 2.5 | f_diff$r1b_4 > 2.5 | f_diff$r1b_5 > 2.5 | f_diff$r1b_6 > 2.5), "DN","NR")))

table(f_diff$k2b_deg<-ifelse((temp$kdm2b1 > 1 | temp$kdm2b2 > 1) & (f_diff$wt > 2.5 | f_diff$kdm2b1 > 2.5 | f_diff$kdm2b2 > 2.5),"UP",
                             ifelse((temp$kdm2b1 < -1 | temp$kdm2b2 < -1) & (f_diff$wt > 2.5 | f_diff$kdm2b1 > 2.5 | f_diff$kdm2b2 > 2.5), "DN","NR")))

f_total<-as.data.frame(f)
summary(idx<-match(rownames(f_total),rownames(f_diff)))
table(f_total$bcor_deg<-f_diff[idx,"bcor_deg"],useNA="ifany")
table(f_total$rab_deg<-f_diff[idx,"rab_deg"],useNA="ifany")
table(f_total$kdm2b_deg<-f_diff[idx,"k2b_deg"],useNA="ifany")

#summary(idx2<-match(rownames(temp3),tss_classified$ensembl))
#temp3$class<-tss_classified[idx2,"class"]

#not define strong for each category for Venn Diagram
#bcor_genes<-rownames(temp3[temp3$bcor_deg=="UP" & rownames(temp3) %in% tss_classified$ensembl[tss_classified$class=="strong"],])
#rab_genes<-rownames(temp3[temp3$rab_deg=="UP" & rownames(temp3) %in% tss_classified$ensembl[tss_classified$class=="strong"],])
#kdm2b_genes<-rownames(temp3[temp3$kdm2b_deg=="UP" & rownames(temp3) %in% tss_classified$ensembl[tss_classified$class=="strong"],])

dim(f_strong<-f_total[rownames(f_total) %in% tss_classified$ensembl[tss_classified$class=="strong"],])
length(bcor_genes<-rownames(subset(f_strong,bcor_deg=="UP")))
length(rab_genes<-rownames(subset(f_strong,rab_deg=="UP")))
length(kdm2b_genes<-rownames(subset(f_strong,kdm2b_deg=="UP")))
...
write.csv(temp2,file=paste0("Figure6_heatmap_table_",ts,".csv"),quote=F, col.names = T, row.names = T)
...
## FPKM Boxplot (Figure 2F)
```{r add_fpkm,eval=T}
#f_sh["ENSG00000163508",]
load("../hESC/tss_classified_081817.rdata")

x<-assays(tss_counts_k36me2)$counts
summary(idx<-match(tss_classified$tx_name,rownames(x)))
all.equal(tss_classified$H3K36me2,as.numeric(x[idx,"H3K36me2_ChIP_zheng_GCCAAT_bothruns.R1_trimmed.fastq.bam"]))
tss_classified$INPUT_H3K36me2<-as.numeric(x[idx,"Input_DNA_zheng_CAGATC_bothruns.R1_trimmed.fastq.bam"])

x<-assays(tsswide_counts_k36me2)$counts
summary(idx<-match(tss_classified$tx_name,rownames(x)))
all.equal(tss_classified$H3K36me2_wide,as.numeric(x[idx,"H3K36me2_ChIP_zheng_GCCAAT_bothruns.R1_trimmed.fastq.bam"]))
tss_classified$INPUT_H3K36me2_wide<-as.numeric(x[idx,"Input_DNA_zheng_CAGATC_bothruns.R1_trimmed.fastq.bam"])
head(tss_classified)

mcols(tss_classified)<-mcols(tss_classified)[,c(1:6,42,7:24,43,25:41)]

summary(idx<-match(tss_classified$ensembl,rownames(f_total)))
tss_classified$H1P_ave_fpkm<-(f_total[idx,"h1p1a"]+f_total[idx,"h1p2a"])/2
tss_classified$BCOR_ave_fpkm<-(f_total[idx,"bcor1a"]+f_total[idx,"bcor2a"])/2
tss_classified$bcor_0_fpkm<-f_total[idx,"bcor_0"]
tss_classified$bcor_1_fpkm<-f_total[idx,"bcor_1"]
tss_classified$bcor_2_fpkm<-f_total[idx,"bcor_2"]
tss_classified$bcor_3_fpkm<-f_total[idx,"bcor_3"]
tss_classified$bcor_4_fpkm<-f_total[idx,"bcor_4"]
tss_classified$bcor_5_fpkm<-f_total[idx,"bcor_5"]
tss_classified$bcor_6_fpkm<-f_total[idx,"bcor_6"]

tss_classified$H1_H1P_ave_fpkm<-(f_total[idx,"H1P1"]+f_total[idx,"H1P2"])/2
tss_classified$H1_RAB_ave_fpkm<-(f_total[idx,"RAB1"]+f_total[idx,"RAB2"])/2
tss_classified$r1b_0_fpkm<-f_total[idx,"r1b_0"]
tss_classified$r1b_1_fpkm<-f_total[idx,"r1b_1"]
tss_classified$r1b_2_fpkm<-f_total[idx,"r1b_2"]
tss_classified$r1b_3_fpkm<-f_total[idx,"r1b_3"]
tss_classified$r1b_4_fpkm<-f_total[idx,"r1b_4"]
tss_classified$r1b_5_fpkm<-f_total[idx,"r1b_5"]
tss_classified$r1b_6_fpkm<-f_total[idx,"r1b_6"]

tss_classified$WT_fpkm<-f_total[idx,"wt"]
tss_classified$KDM2B_ave_fpkm<-(f_total[idx,"kdm2b1"]+f_total[idx,"kdm2b2"])/2
tss_classified$kdm2b1_fpkm<-f_total[idx,"kdm2b1"]
tss_classified$kdm2b2_fpkm<-f_total[idx,"kdm2b2"]

#Add Classifications based on shRNA Data
tss_classified$bcorSH_deg<-ifelse(tss_classified$BCOR_ave_fpkm > 2.5 & tss_classified$ensembl %in% degs_bcor_up,"UP",
                           ifelse(tss_classified$H1P_ave_fpkm > 2.5 & tss_classified$ensembl %in% degs_bcor_dn,"DN",
                           ifelse(!(tss_classified$ensembl %in% degs_bcor_up | tss_classified$ensembl %in% degs_bcor_dn),"NR","XX")))

table(tss_classified$bcorSH_deg)
table(tss_classified[tss_classified$class=="strong"]$bcorSH_deg)

tss_classified$k2bKO_deg<-ifelse(tss_classified$KDM2B_ave_fpkm > 2.5 & tss_classified$ensembl %in% degs_k2bKO_up,"UP",
...
table(tss_classified$k2bKO_deg)
table(tss_classified[tss_classified$class=="strong"]$k2bKO_deg)

tss_classified$rabSH_deg<-ifelse(tss_classified$H1_RAB_ave_fpkm > 2.5 & tss_classified$ensembl %in% degs_rab_up,"UP",
                           ifelse(tss_classified$H1_H1P_ave_fpkm > 2.5 & tss_classified$ensembl %in% degs_rab_dn,"DN",
                           ifelse(!(tss_classified$ensembl %in% degs_rab_up | tss_classified$ensembl %in% degs_rab_dn),"NR","XX")))
table(tss_classified$rabSH_deg)
table(tss_classified[tss_classified$class=="strong"]$rabSH_deg)


#Add Classifications based on timecourse Data
tss_classified$bcor_deg<-f_total[idx,"bcor_deg"]
tss_classified$rab_deg<-f_total[idx,"rab_deg"]
tss_classified$kdm2b_deg<-f_total[idx,"kdm2b_deg"]
View(as.data.frame(mcols(tss_classified[tss_classified$class=="strong"])))
write.csv(mcols(tss_classified),file=paste0("chip_enrichment_and_fpkm_data_",ts,".csv"),quote=F,row.names = F)
```


## Mean density plots and boxplots (Figure 2G & Sup2H)


## GO Analysis on Expression Data
### KDM2B GO terms
```{r}
...
# Analysis of ChIP data based on Gene Expression Changes
## Define WT fileset (MSI)
...
tss_temp<-tss_classified


chip_t_test<-function(cls="strong",cat1="UP",cat2="NR",signal="BCOR_wide",control="INPUT_wide") {

x1 <-log2(mcols(tss_temp)[tss_temp$class==cls & tss_temp$bcorSH_deg==cat1,signal]+1)
x1c<-log2(mcols(tss_temp)[tss_temp$class==cls & tss_temp$bcorSH_deg==cat1,control]+1)
#x1<-ifelse(x1-x1c> 0, x1-x1c, NA)
x1<-x1-x1c

x2<-log2(mcols(tss_temp)[tss_temp$class==cls & tss_temp$bcorSH_deg==cat2,signal]+1)
x2c<-log2(mcols(tss_temp)[tss_temp$class==cls & tss_temp$bcorSH_deg==cat2,control]+1)
#x2<-ifelse(x2-x2c > 0, x2-x2c,NA)
x2<-x2-x2c

#x1<-x1[is.finite(x1)]
#x2<-x2[is.finite(x2)]
val_t <- suppressWarnings(t.test(x1,x2,alternative = "two.sided",paired=F)$p.value)
#val_t <- round(-1*log10(suppressWarnings(t.test(x1,x2,alternative = "two.sided",paired=F)$p.value)),3)
#val_k <- round(-1*log10(suppressWarnings(ks.test(x1,x2,exact=T)$p.value)),3)
#title<-paste0(signal,cat1,cat2,":  t.test=",val_t,", ks.test= ",val_k)
#boxplot(list(x1,x2),main=title)     
return(val_t)

#return(suppressWarnings(t.test(na.omit(log2(x1[,1]+1)),na.omit(log2(x2[,1]+1)))$p.value))
}

chip_t_test(cls="strong",cat1="UP",cat2="NR",signal="RNF2_wide")

options("scipen"=-100, "digits"=4)
x<-colnames(mcols(tss_temp)) %>% grep("_wide",.,value=T) %>% grep("INPUT",.,value=T,invert=T) %>% grep("CGI",.,value=T,invert=T)
#x<-c("BCOR_wide","KDM2B_wide","RNF2_wide","PCGF1_wide","RYBP_wide","H2AUb_wide","SUZ12_wide","H3K27me3_wide","CBX2_wide","DNAse1_wide")

  
df<-matrix(nrow=length(x),ncol=3)
colnames(df)<-c("UP vs DN","UP vs NR","DN vs NR")
#rownames(df)<-colnames(mcols(tss_temp))[5:22]
rownames(df)<-x
...
for (j in rownames(df)) {
  ...
  df[j,i]<-ifelse(j=="H3K36me2_wide",chip_t_test(cat1=c1,cat2=c2,signal=j,cls="strong",control="INPUT_H3K36me2_wide"),chip_t_test(cat1=c1,cat2=c2,signal=j,cls="strong",control="INPUT_wide"))
  #df[j,i]<-chip_t_test(cat1=c1,cat2=c2,signal=j,cls="strong")
}
}
...
round(df,12)
write.csv(round(df,12),file="strong_S2G.csv",quote=F)
...
chip_t_test(cls="weak",cat1="UP",cat2="NR",signal="RNF2_wide")
df<-matrix(nrow=length(x),ncol=3)
colnames(df)<-c("UP vs DN","UP vs NR","DN vs NR")
#rownames(df)<-colnames(mcols(tss_temp))[5:22]
rownames(df)<-x
...
for (j in rownames(df)) {
  ...
  #print(paste0("c1:  ",c1," c2:  ",c2," ks.test:"))
  #print(paste0("c1:  ",c1," c2:  ",c2," ks.test:",ks_chip_test(cat1=c1,cat2=c2)))
  df[j,i]<-ifelse(j=="H3K36me2_wide",chip_t_test(cat1=c1,cat2=c2,signal=j,cls="weak",control="INPUT_H3K36me2_wide"),chip_t_test(cat1=c1,cat2=c2,signal=j,cls="weak",control="INPUT_wide"))
}
}

options("scipen"=999, "digits"=12)
round(df,12)
write.csv(round(df,12),file="weak_S2G.csv",quote=F)
...
temp_plot<-wide_ks[,1:3] %>% as.data.frame %>% 
  ...
  ggplot(aes(x=cat,y=chip)) + geom_tile(aes(fill=pval))+
...
ggsave(temp_plot,file="Figure_S2H_based_onShRHA.pdf",height=5,width=6)

```


## GO terms for differentially Expressed genes (Sup2G)
...
# Loss of Function ChIP Analysis - BCOR-R, RNF2-R and KDM2B-KO

## Create bigWig files normalized to mappable counts for Figure 4D and Figure 6F
```{r}
bam2bw<-function(bam_file) { 
qfilter=4
param <- ScanBamParam(what="mapq",flag = scanBamFlag(isUnmappedQuery = FALSE))
size<-countBam(bam_file,param=param,)$records
print(paste0("The file ",bam_file," has ",size," reads."))
temp<-readGAlignments(bam_file,param=param)
temp.cov<-coverage(temp[mcols(temp)$mapq >= qfilter],weight=1e6/size)
print(bigwig_file<-paste0(strsplit(basename(bam_file),"\\.")[[1]][1],"_q",qfilter,"_scaled.bigWig"))
export(temp.cov,bigwig_file)
}

bam2bw("../hESC/H1_KDM2Bplus_P038_Index5_S16.R1_trimmed.fastq.bam")
bam2bw("../hESC/H1_KDM2Bminus_P038_Index3_S14.R1_trimmed.fastq.bam")

bam2bw("../hESC/H1_BCORplus_P038_Index4_S15.R1_trimmed.fastq.bam")
bam2bw("../hESC/H1_BCORminus_P038_Index2_S13.R1_trimmed.fastq.bam")

bam2bw("../hESC/H1_wt_BCOR_P36_Index15_S14.fastq.bam")
bam2bw("../hESC/H1_k2bko_BCOR_P36_Index17_S16.fastq.bam")

bam2bw("../hESC/H1_INPUTplus_P038_Index8_S19.R1_trimmed.fastq.bam")
bam2bw("../hESC/H1_INPUTminus_P038_Index6_S17.R1_trimmed.fastq.bam")


temp_k_plus<-readGAlignments("../hESC/H1_KDM2Bplus_P038_Index5_S16.R1_trimmed.fastq.bam",param=param)
temp_k_minus<-readGAlignments("../hESC/H1_KDM2Bminus_P038_Index3_S14.R1_trimmed.fastq.bam",param=param)

table(mcols(temp_k_plus)$mapq)/length(temp_k_plus)
table(mcols(temp_k_minus)$mapq)/length(temp_k_minus)
rm(temp)
rm(temp_k_plus)
rm(temp_k_minus)
```

## Measure counts for All ChIP experiments in LOF studies for bound targets
...
## Heatmap plot for BCOR ChIP in WT vs KDMB2B KO cell lines (Figure 4E)


## Metaplots for BCOR_ONLY vs KDM2B_ONLY gene sets (Figure 4G)
```{r figure4G}
head(degs.k2bKO_up)
load("bound_w_counts.rdata")

#"Mon Oct 16 15:43:10 2017"  -- as of today the bound matrix still has the shRNA calls in the deg_bcor field.  Even though I have switched over to bcorSH_deg 
#nomenclature to keep track of the two gene sets but for now I am going to keep the bound_w_counts data separate.  Bound also does not have the H3K36me2 input
#data as one of the columns.  106/197/415
...
## Calculate p-values from tss_temp
```{r}
fig4_chip_t_test<-function(cls="strong",cat1="bcor_only",cat2="kd2b_only",signal="BCOR_wide",control="INPUT_wide") {

x1 <-log2(mcols(tss_temp)[tss_temp$class==cls & tss_temp$k_vs_b==cat1,signal]+1)
x1c<-log2(mcols(tss_temp)[tss_temp$class==cls & tss_temp$k_vs_b==cat1,control]+1)
#x1<-ifelse(x1-x1c> 0, x1-x1c, NA)
x1<-x1-x1c

x2<-log2(mcols(tss_temp)[tss_temp$class==cls & tss_temp$k_vs_b==cat2,signal]+1)
x2c<-log2(mcols(tss_temp)[tss_temp$class==cls & tss_temp$k_vs_b==cat2,control]+1)
#x2<-ifelse(x2-x2c > 0, x2-x2c,NA)
x2<-x2-x2c
...
my_comparisons <- list( c("bcor_only","k2b_bcor"), c("bcor_only","kd2b_only"), c("k2b_bcor","kd2b_only"))
x<-colnames(mcols(tss_temp)) %>% grep("_wide",.,value=T) %>% grep("INPUT",.,value=T,invert=T) %>% grep("CGI",.,value=T,invert=T)

options("scipen"=-100, "digits"=4)
df<-matrix(nrow=length(x),ncol=3)
colnames(df)<-sapply(my_comparisons,function(x) paste0(x[1],":",x[2]))
#rownames(df)<-colnames(mcols(tss_temp))[5:22]
rownames(df)<-x

for (j in rownames(df)) {
  for (i in colnames(df)) {
  c1=sapply(strsplit(i,":"), function(x) x[1])
  c2=sapply(strsplit(i,":"), function(x) x[2])
  #print(paste0("c1:  ",c1," c2:  ",c2," ks.test:"))
  #print(paste0("c1:  ",c1," c2:  ",c2," ks.test:",ks_chip_test(cat1=c1,cat2=c2)))
  df[j,i]<-ifelse(j=="H3K36me2_wide",fig4_chip_t_test(cat1=c1,cat2=c2,signal=j,cls="strong",control="INPUT_H3K36me2_wide"),fig4_chip_t_test(cat1=c1,cat2=c2,signal=j,cls="strong",control="INPUT_wide"))
}
}

options("scipen"=999, "digits"=12)
round(df,12)
write.csv(round(df,12),file="strong_S4.csv",quote=F)
```
## Define gene expression changes based on timecourse data for Venn Diagram (Figure 6C)

## Heatmap for timecourse data (Figure 6C)

## Heatmap plot for BCOR and KDM2B ChIPs in RNF2-R Cells +/- DOX (Figure 6E)

## GO terms for differentially expressed genes in RNF2-R timecourse (Figure Sup6C)

## Boxplots for BCOR and KDM2B ChIP counts in RNF2-R d0 vs d6 for BCOR responsive vs KDM2B responsive genes (Figure Sup6D)
...
chip_df<- bound_strong_sort[,"e_wide",drop=F]
...
system.time(k2bko<-tornado(tss_classified[idx],pad=9000,k2bko_fileset,ord=0,color="#00441B",window=100)) #green
...
#temp is defined as difference between t=0 below
summary(idx<-match(bound_strong_sort$ensembl,rownames(temp)))
pheatmap(temp[idx,c(1:6,13,14)],cluster_cols=F,cluster_rows=F,show_rownames = F,scale="none",col=colors)

write.csv(temp[idx,c(1:6,13,14)],paste0("Figure4E_heatmap_table_",ts,".csv"),quote=F, row.names = T)
...
ks_chip_test<-function(cls="strong",cat1="UP",cat2="NR",signal="BCOR_wide",control="INPUT_wide") {


x1<-log2(mcols(tss_classified)[tss_classified$class==cls & tss_classified$bcor_deg==cat1,signal])
x1c<-log2(mcols(tss_classified)[tss_classified$class==cls & tss_classified$bcor_deg==cat1,control])
x1<-ifelse(x1-x1c> 0, x1-x1c, 0)

x2<-log2(mcols(tss_classified)[tss_classified$class==cls & tss_classified$bcor_deg==cat2,signal])
x2c<-log2(mcols(tss_classified)[tss_classified$class==cls & tss_classified$bcor_deg==cat2,control])
x2<-ifelse(x2-x2c > 0, x2-x2c,0)
...
k_v_b<-
#easy just to make a variable in tss_temp to stratify these 3 groups.
table(tss_temp$k_vs_b<-ifelse(tss_temp$ensembl %in% kd2b_only$ensembl,"kd2b_only",
                        ifelse(tss_temp$ensembl %in% bcor_only$ensembl,"bcor_only",
                               ifelse(tss_temp$ensembl %in% k2b_bcor$ensembl,"k2b_bcor",
                                      ifelse(tss_temp$ensembl %in% not_up$ensembl,"not_up",NA)))),useNA="ifany")
...
my_comparisons <- list( c("strongPLUS","strongMINUS"), c("weakPLUS","weakMINUS"))
#option 4
mcols(bound) %>% as.data.frame() %>%
 # dplyr::filter(class=="strong") %>% 
  dplyr::select(kd2b_v_bcor,class,BCOR_PLUS_wide_cpm,BCOR_MINUS_wide_cpm,KDM2B_PLUS_wide_cpm,KDM2B_MINUS_wide_cpm) %>% 
 ...
 # dplyr::mutate(chip=factor(chip,levels=c("BCOR_WT","BCOR_K2BKO","BCOR_PLUS","BCOR_MINUS","KDM2B_PLUS","KDM2B_MINUS"))) %>% 
  dplyr::filter(is.finite(cpm)) %>% 
  dplyr::filter(antibody=="KDM2B") %>% 
  ggplot(aes(x=x,y=cpm,fill=chip)) + 
  geom_boxplot(outlier.size = 0.5) +
  facet_wrap(~ kd2b_v_bcor,ncol=4) +
 # facet_grid(antibody ~ kd2b_v_bcor) +
  stat_compare_means(paired = T,method="t.test",comparisons = my_comparisons) +
 # facet_wrap(c("kd2b_v_bcor", "class"), labeller = "label_both",nrow=1) +
...
```
 ...
 Option 5 - Megaboxplot for kdm2b_V_bcor
```{r}
x<-colnames(mcols(tss_classified)) %>% grep("_wide",.,value=T)
x<-as.matrix(mcols(tss_classified)[tss_classified$class=="strong",c(x)])
x<-as.data.frame(log2(x+1))

all.equal(rownames(tss_temp[tss_temp$class=="strong"]),rownames(tss_classified[tss_classified$class=="strong"]))
x$k_v_b<-tss_temp$k_vs_b[tss_temp$class=="strong"]

x2<-cbind(x[,c(4:8,10)]-x[,2],x[,11,drop=F]-x[,3,drop=F],x[,c(12:19)]-x[,2],x[,"k_v_b",drop=F])

table(x2$k_v_b,useNA="ifany")
...
my_comparisons <- list( c("bcor_only","k2b_bcor"), c("bcor_only","kd2b_only"), c("k2b_bcor","kd2b_only"))
x2 %>% as.data.frame %>%  tidyr::gather(chip,enrichment,-k_v_b) %>% 
  dplyr::filter(k_v_b!="not_up") %>% 
  dplyr::mutate(k_v_b=factor(k_v_b,levels=c("bcor_only","k2b_bcor","kd2b_only"))) %>% 
  dplyr::mutate(chip=gsub("_wide","",chip)) %>% 
  dplyr::mutate(chip=factor(chip,levels=gsub("_wide","",colnames(x2)))) %>% 
  ggplot(aes(x=k_v_b,y=enrichment,fill=k_v_b)) +stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") +
    scale_fill_manual(values=c("#2A6042","gray","#2E4099")) + facet_wrap(~chip,scales="free",ncol=5) +
    stat_compare_means(paired = F,method="t.test",comparisons = my_comparisons) +
    theme_bw() +theme(strip.background = element_blank())

ggsave(file="Figure_S4H_boxplot_based_onshRNA.pdf",height=8.5,width=11)
...
```{r}
length(bups<-subset(tss_classified,bcor_deg=="UP" & class=="strong"))
length(bnrs<-subset(tss_classified,bcor_deg=="NR" & class=="strong"))
length(bdns<-subset(tss_classified,bcor_deg=="DN" & class=="strong"))

length(bupw<-subset(tss_classified,bcor_deg=="UP" & class=="weak"))
length(bnrw<-subset(tss_classified,bcor_deg=="NR" & class=="weak"))
length(bdnw<-subset(tss_classified,bcor_deg=="DN" & class=="weak"))

bcor_granges<-GRangesList(list(bups,bnrs,bdns,bupw,bnrw,bdnw))
names(bcor_granges)<-c("bups","bnrs","bdns","bupw","bnrw","bdnw")
lapply(bcor_granges,length)

length(bs<-subset(tss_classified,bcor_deg=="UP" & kdm2b_deg!="UP" & class=="strong"))
length(ks<-subset(tss_classified,bcor_deg!="UP" & kdm2b_deg=="UP" & class=="strong"))
length(bks<-subset(tss_classified,bcor_deg=="UP" & kdm2b_deg=="UP" & class=="strong"))

length(bw<-subset(tss_classified,bcor_deg=="UP" & kdm2b_deg!="UP" & class=="weak"))
length(kw<-subset(tss_classified,bcor_deg!="UP" & kdm2b_deg=="UP" & class=="weak"))
length(bkw<-subset(tss_classified,bcor_deg=="UP" & kdm2b_deg=="UP" & class=="weak"))
...
save(bcor_v_kdm2b_granges,bcor_granges,file="meta_granges.rdata")

```

#test top genes
```{r}
subset_tss <-function (n=50) {

bups_temp<-bups
bups_temp$lfc<-log2(bups_temp$bcor_6_fpkm+1)-log2(bups_temp$bcor_0_fpkm+1)
bups_temp<-bups_temp[with(bups_temp,order(-lfc))]
bups_temp<-bups_temp[1:n]

bdns_temp<-bdns
bdns_temp$lfc<-log2(bdns_temp$bcor_6_fpkm+1)-log2(bdns_temp$bcor_0_fpkm+1)
bdns_temp<-bdns_temp[with(bdns_temp,order(lfc))]
head(bdns_temp$lfc)
bdns_temp<-bdns_temp[1:n]

bnrs_temp<-sample(bnrs,n)
#bnrs_temp<-bnrs
bnrs_temp$lfc<-log2(bnrs_temp$bcor_3_fpkm+1)-log2(bnrs_temp$bcor_0_fpkm+1)

gr<-GRangesList(list(bups_temp,bnrs_temp,bdns_temp))
names(gr)<-c(paste0("up_top",n),paste0("nr_top",n),paste0("dn_top",n))
return(gr)
}

b_top_50<-subset_tss(50)
b_top_100<-subset_tss(100)
lapply(b_top_100,length)

lfc_list<-list(up_full=bcor_full[["up"]]$lfc,nr_full=bcor_full[["nr"]]$lfc,dn_full=bcor_full[["dn"]]$lfc,
               up_100=b_top_100[["up_top100"]]$lfc,nr_100=b_top_100[["nr_top100"]]$lfc,dn_100=b_top_100[["dn_top100"]]$lfc,
               up_50=b_top_50[["up_top50"]]$lfc,nr_50=b_top_50[["nr_top50"]]$lfc,dn_50=b_top_50[["dn_top50"]]$lfc)

b_mix<-GRangesList(list(b_top_50[["up_top50"]],b_top_100[["up_top100"]],bcor_full[["nr"]],b_top_100[["dn_top100"]],b_top_50[["dn_top50"]]))
names(b_mix)<-c("up_top50","up_top100","nr","dn_top100","dn_top50")
lapply(b_mix,length)
save(b_mix,file="b_mix.rdata")
boxplot(lfc_list)
save(b_top_50,b_top_100,file="b_top_50_day3.rdata")

nr_mix<-GRangesList(list(b_top_50[["nr_top50"]],b_top_100[["nr_top100"]],bcor_full[["nr"]]))
names(nr_mix)<-c("nr_top50","nr_top100","nr")
lapply(nr_mix,length)
save(nr_mix,file="nr_mix.rdata")
...
```{r}
x<-
data.frame(chip=colnames(mcols(tss_classified)) %>% grep("_wide",.,value=T))

myttest<-function(chip) {
  a=log2(mcols(bups)[,chip])
  a<-a[is.finite(a)]
  b=log2(mcols(bnrs)[,chip])
  b<-b[is.finite(b)]
  -1*log10(t.test(a,b,alternative = "two.sided",paired=F)$p.value)
}

x$pval<-1
for ( i in 1:19) { x[i,"pval"]<- myttest(x[i,"chip"])}
```
...
x<-as.matrix(mcols(tss_classified)[tss_classified$class=="strong",c(x)])
x<-as.data.frame(log2(x+1))
x$bcor_deg<-tss_classified$bcorSH_deg[tss_classified$class=="strong"]
x2<-cbind(x[,c(4:8,10)]-x[,2],x[,11,drop=F]-x[,3,drop=F],x[,c(12:19)]-x[,2],x[,"bcor_deg",drop=F])
...
my_comparisons <- list( c("UP","NR"), c("NR","DN"), c("UP","DN"))
temp2_plot<-x2 %>% as.data.frame %>%  tidyr::gather(chip,enrichment,-bcor_deg) %>% 
  dplyr::filter(bcor_deg!="XX") %>% 
  dplyr::mutate(bcor_deg=factor(bcor_deg,levels=c("UP","NR","DN"))) %>% 
  dplyr::mutate(chip=gsub("_wide","",chip)) %>% 
  dplyr::mutate(chip=factor(chip,levels=gsub("_wide","",colnames(x2)))) %>% 
  ggplot(aes(x=bcor_deg,y=enrichment,fill=bcor_deg)) +stat_summary(fun.data = min.mean.sd.max, geom = "boxplot") +
    scale_fill_manual(values=c("#EF3A38","#BFBFBF","#A2AED9","cyan"),name="") +
    stat_compare_means(paired = F,method="t.test",comparisons = my_comparisons) +
    facet_wrap(~chip,scales="free",ncol=5) +theme_bw() +theme(strip.background = element_blank())

ggsave(temp2_plot,file="Figure_S2H_boxplot_based_onshRNA.pdf",height=8.5,width=11)
...
check natalia's t.tests
```{r}
nat<-read.csv("../hESC/H1/chip_enrichment_and_fpkm_data_090517.csv",stringsAsFactors=F)
table(nat)
```

```{r}
x<-colnames(mcols(tss_temp)) %>% grep("_fpkm",.,value=T) %>% grep("ave",.,value=T,invert=F) 
x<-tss_temp[,c(x[c(1,2)],"class")]

head(x)
x %>%  mcols %>%  as.data.frame() %>% 
  tidyr::gather(time,fpkm,-class) %>% 
  #tidyr::gather(time,fpkm,-class,-bcor_0_fpkm) %>% 
 # dplyr::rowwise() %>% 
  #dplyr::mutate(time=factor(stringr::str_split(time,"_",simplify = T)[1])) %>% 
  #dplyr::mutate(time=sapply(strsplit(time,"_"),function(x) x[1])) %>% 
  #dplyr::ungroup() %>% 
  #dplyr::mutate(time=factor(time,levels=c("H1P","BCOR"))) %>% 
  dplyr::mutate(time=factor(gsub("_","",substr(time,1,4)),levels=c("H1P","BCOR"))) %>% 
  #dplyr::mutate(time=gsub("_","",substr(time,1,4))) %>% 
  #dplyr::mutate(log2FPKM=log2(fpkm+0.1)-log2(bcor_0_fpkm+0.1)) %>% 
  dplyr::mutate(log2FPKM=log2(fpkm+0.1)) %>% 
  dplyr::mutate(class=factor(class,levels=c("strong","weak","nobinding"))) %>% 
  ggplot(aes(x=time,y=log2FPKM)) + geom_boxplot(size=0.2) + facet_wrap(~class) +
  scale_fill_manual(values=c("#A2AED9","#EF3A38")) +
   stat_compare_means(label = "p.format",paired = T,method="wilcox.test") +
  theme_bw() + theme(strip.background = element_blank(),
                     axis.text.x = element_text(angle = 90, hjust = 1),
                     axis.ticks.y=element_blank(),
                     panel.grid.major=element_blank(),
...
ggsave("Figure2F.pdf",width=6,height=4)

x2<-x %>%  mcols %>%  as.data.frame() %>% 
  tidyr::gather(time,fpkm,-class) %>% 
  #dplyr::mutate(time=factor(substr(time,6,6))) %>% 
  dplyr::mutate(time=factor(gsub("_","",substr(time,1,4)),levels=c("H1P","BCOR"))) %>% 
  dplyr::mutate(log2FPKM=log2(fpkm+0.1)) %>% 
  dplyr::mutate(class=factor(class,levels=c("strong","weak","nobinding"))) 

length(a<-x2[x2$class=="nobinding" & x2$time=="H1P","log2FPKM"])
length(b<-x2[x2$class=="nobinding" & x2$time=="BCOR","log2FPKM"])
wilcox.test(a,b,paired=T)$p.value
ks.test(a,b)$p.value
...
#KDM2B GO terms - Address concerns of reviewer #1 
# The fact that KDM2B-KO hESC display broad transcriptional changes, including "housekeeping" functions, is perhaps not that surprising, since KDM2B is not only found within PRC1 complexes and can be bound to almost all CpG islands. How does the gene ontology analysis looks like for genes upregulated in KDM2B-KO and belonging to the strong binding group and/or also bound by other PRC components (e.g. RNF2)?. Are these subset of KDM2B targets also enriched in housekeeping function or in developmental ones instead?. If the later, are they different from the BCOR target categories?.

```{r}


temp<-tss_classified[tss_classified$ensembl %in% expressed_genes]

temp <- as.data.frame(mcols(temp))
temp<-temp[,c("ensembl","hgnc","class","RNF2_wide","k2bKO_deg")]
table(temp$k2bKO_deg)

temp.up<-as.numeric(expressed_genes %in% temp$ensembl[temp$k2bKO_deg=="UP"])
names(temp.up)<-expressed_genes
table(temp.up)
pwf.temp.up<-nullp(temp.up,bias.data=bd)
GO.wall.temp.up<-goseq(pwf.temp.up,gene2cat=goa_human.list)
head(GO.wall.temp.up)

#now divide up
boxplot(log2(temp$RNF2_wide[temp$k2bKO_deg=="UP"]) ~ temp$class[temp$k2bKO_deg=="UP"])
temp.strong.up<-as.numeric(expressed_genes %in% temp$ensembl[temp$k2bKO_deg=="UP" & temp$class=="strong"])
names(temp.strong.up)<-expressed_genes
table(temp.strong.up)
pwf.temp.strong.up<-nullp(temp.strong.up,bias.data=bd)
GO.wall.temp.strong.up<-goseq(pwf.temp.strong.up,gene2cat=goa_human.list)
head(GO.wall.temp.strong.up,20)
write.csv(GO.wall.temp.strong.up[GO.wall.temp.strong.up$over_represented_pvalue < 0.05,],file="GO.wall.temp.strong.up.csv",quote=F,row.names = F)
revigo.temp_strong_up<-read.csv("revigo_temp_strong_up.csv",stringsAsFactors = F,header=T)
revigo.temp_strong_up$class<-"strong"

temp.weak.up<-as.numeric(expressed_genes %in% temp$ensembl[temp$k2bKO_deg=="UP" & temp$class=="weak"])
names(temp.weak.up)<-expressed_genes
table(temp.weak.up)
pwf.temp.weak.up<-nullp(temp.weak.up,bias.data=bd)
GO.wall.temp.weak.up<-goseq(pwf.temp.weak.up,gene2cat=goa_human.list)
head(GO.wall.temp.weak.up,20)
write.csv(GO.wall.temp.weak.up[GO.wall.temp.weak.up$over_represented_pvalue < 0.05,],file="GO.wall.temp.weak.up.csv",quote=F,row.names = F)
revigo.temp_weak_up<-read.csv("revigo_temp_weak_up.csv",stringsAsFactors = F,header=T)
revigo.temp_weak_up$class<-"weak"

temp %>% 
  dplyr::filter(k2bKO_deg=="UP") %>% 
  dplyr::select(class,RNF2_wide)  %>% 
  dplyr::mutate(RNF2_wide=log2(RNF2_wide + 1)) %>% 
  dplyr::mutate(class=factor(class,levels=c("strong","weak","nobinding"))) %>% 
  ggplot(aes(x=class,y=RNF2_wide,fill=class)) + geom_boxplot() +
  xlab("") + ylab("log2 (RNF2_wide + 1)") +
  scale_fill_manual(values=c("#993366","#D37BA7","gray"),guide=FALSE) +theme_bw() +
    theme(strip.background = element_blank(), 
        panel.grid.minor.y = element_blank(),
        panel.grid.major.y = element_blank(),
        panel.grid.major.x = element_blank())

rbind(revigo.temp_strong_up,revigo.temp_weak_up) %>% 
  dplyr::select(description,log10.p.value,class) %>% 
  dplyr::mutate(log10.p.value = -1* log10.p.value) %>% 
  dplyr::filter(log10.p.value > 4) %>% 
  dplyr::mutate(class=factor(class,levels=rev(c("strong","weak")))) %>% 
  dplyr::arrange(class,log10.p.value) %>% 
  dplyr::mutate(description=factor(description,levels=description)) %>% 
ggplot(aes(x=description,y=log10.p.value,fill=class)) + 
  geom_bar(stat="identity") +
  scale_fill_manual(values=c("#D37BA7","#993366"),guide=FALSE) +
  coord_flip() + ylab("-Log10( p-value)") +
  theme_bw() + ggtitle("KDM2B_UP genes by class")
  theme(strip.background = element_blank(), 
        panel.grid.minor.y = element_blank(),
        panel.grid.major.y = element_blank(),
        panel.grid.major.x = element_blank())
