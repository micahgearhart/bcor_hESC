...
pol2_fileset<-new("fileset", filename=c( "../estaras_Ser5P_RNAPII_untreated_SRR1745499.R1_trimmed.fastq.bam","../estaras_Ser7P_RNAPII_untreated_SRR1745503.R1_trimmed.fastq.bam"),
...
rna_fileset<-new("fileset", filename=c( "../H1P1A_CGATGT.bam","../BCOR1A_TGACCA.bam"),
           labels=c("H1P","shBCOR"))
...
```{r}
summary(degs_bcor_up<-rownames(subset(res_bcorsh,padj< alpha & log2FoldChange > lfc_thres)))
summary(degs_bcor_nc<-rownames(subset(res_bcorsh,padj> alpha & abs(log2FoldChange) <= lfc_thres)))
summary(degs_bcor_dn<-rownames(subset(res_bcorsh,padj< alpha & log2FoldChange < -1*lfc_thres)))
